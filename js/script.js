...
var revealCare = {
	    ...
	    success:function(res)
	    {
	        	arr = $.parseJSON( res );
	      		console.log(arr[0]);
	       		...
	       		$("#name").text(arr[0]['ID']['S']);
			
			$("#address").text(arr[0]['Address']['S']);
			//google maps
	...
	loadAboutDataFacility: function(value){
		var arr=[];
	    $.ajax({
	    type:'POST',
	    url: revealCare.reviewUrl+'About.php?searchKey='+value,
	    success:function(res)
	    {
	        	arr = $.parseJSON( res );
	      		console.log(arr[0]);
	       		...
	       		$("#fname").text(arr[0]['ID']['S']);
			
			$("#faddress").text(arr[0]['Address']['S']);
			//google maps
			...
			{		
	        		...
	        		success:function(res)
				{
					reviewArr = $.parseJSON(res);
					console.log(res);
					$("#freviews").append("<br><br>" + reviewArr[0]['From']['S']);
					$("#freviews").append("<br>Quality: "+String(reviewArr[0]['Quality']['N']));
					$("#freviews").append(" Value: "+String(reviewArr[0]['Value']['N']));
					$("#freviews").append(" Cost: "+String(reviewArr[0]['Cost']['N']));
					$("#freviews").append("<br>"+reviewArr[0]['Review']['S']);
	...
	init: function(){
		...
		*/
		...
		$('#doctor_search_div').click(function(){
			var arr=[];
			$.ajax({
			type:'POST',
			...
			{
				   arr = $.parseJSON( res );
				   $('#doctor_search_id').autocomplete({
						source: arr,
						select: function(event, ui) {
							console.log(ui);
							revealCare.loadAboutData(ui.item.value);
						}
					});
			}
			});
		...
		$('#facility_search_div').click(function(){
			var arr=[];
			$.ajax({
			type:'POST',
			...
			{
				   arr = $.parseJSON( res );
				   $('#facility_search_id').autocomplete({
						source: arr,
						select: function(event, ui) {
							console.log(ui);
							revealCare.loadAboutDataFacility(ui.item.value);
					}
					});
			}
			});
